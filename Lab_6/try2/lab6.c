...
uint8_t AlarmSound = 0; //0-> Tone generator. 1-> Radio Stations
...
volatile uint16_t current_fm_freq = 9770;//9990;
volatile uint16_t current_am_freq;
volatile uint16_t current_sw_freq;
uint8_t  current_volume;

uint8_t playingRadio = 0; //states what it's doing
uint8_t RadioOnOff=0; //whether it should be on/off
uint8_t radioToggled = 0; //for button toggled
uint8_t freqChanged=0;


// See https://www.sparkfun.com/datasheets/Components/General/Si4734-35-C40.pdf
// on pg. 29 for command
void DisableRadio(){
	set_property(0x4001,0x003F);
	playingRadio = 0;
}
// See https://www.sparkfun.com/datasheets/Components/General/Si4734-35-C40.pdf
// on pg. 29 for commandx
void EnableRadio(){
    ...
    if(chk_buttons(BttnNum)){ //If we read button input
      if(BttnNum == 7){ //7th button is pressed
        ...
        buttons[3] = 1; //button array
				//EnableRadio();//for radion testing (ON)
        //Volume
      }
      else if(BttnNum == 2){ //2nd button is pressed
        buttons[2] = 1; //button array
        //Radio mode
				//DisableRadio(); //for radion testing (OFF)
        RadioOnOff ^= 1;
				ChangeAlarmTime = 0;
				ChangeTime = 0;
				radioToggled = 1;
      }
			else if(BttnNum == 1){ //1st button is pressed
...
void RadioHandler(){
	if(AlarmSounding == 0 && RadioOnOff == 1){
		playingRadio = 1;
	}
	if(RadioOnOff == 0){
		//Change and save station
		playingRadio = 0;
	}
}
...
void bargraph_updater(){
  ...
  // }]
	...
	else if(AlarmSound == 1){//increment/decrement by 4
    output = 0b10000000;
  }
  else{
    output = 0b00000000;
...
void Encoder_Data(){
  ...
  if(currL_Encoder == 0b11 && prevL_Encoder == 0b01){
    ...
    if(ChangeAlarmTime){AlarmHrs += 1;}
		if(playingRadio){current_fm_freq += 20; freqChanged = 1;}
...
void RadiodigParser(uint16_t val){
  uint8_t Thousands;
  uint8_t Hundreds;
  uint8_t Tens;
  uint8_t Ones;

	//These are shifted values
  Ones = (val/10) % 10;
  segment_data[0] = segMapper(Ones);

	Tens = (val/100) % 10;
	segment_data[1] = (segMapper(Tens));
	segment_data[1] &= ~(1<<7);

	Hundreds = (val/1000) % 10;
	segment_data[3] = segMapper(Hundreds);

	Thousands = (val/10000) % 10;
	segment_data[4] = segMapper(Thousands);
}
void AlarmSetLED(){
  //DDRA = 0xFF;
  asm volatile("nop");
...
int main(){
  ...
  init_twi();
	radio_init();
	current_fm_freq = 9770; //arg2, arg3: 99.9Mhz, 200khz steps

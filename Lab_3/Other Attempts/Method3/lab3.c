...
ISR(TIMER0_OVF_vect){
  ...
  //asm("nop");
  //asm("nop");
  prev_PORTA = PORTA; //->throws off buttons!
  prev_PORTC = PORTC; //->disables buttons! it turns on individual segments
...
int main(){
  ...
  while(1){
    ...
    //Interpret encoder information

    //Check to see if value is below 0

    //Check to see if value is above 1023
